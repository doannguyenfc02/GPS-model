    ...
    public class SignalProcessingService
    {
        ...
        public async Task<string> ProcessFile(IFormFile file, int numImages, double fs, double time)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var finalResponse = await client.GetStringAsync("http://127.0.0.1:5000/upload/completed");
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    foreach (var classification in responseData.classification_output)
                    {
                        signalData.Spectrograms.Add(new Spectrogram
                        {
                            Data = Convert.FromBase64String((string)classification.image_base64),
                            Description = (string)classification.predicted_class
                        });
                    }
